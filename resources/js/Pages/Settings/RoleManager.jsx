...
export default function RoleManager({ auth, roles: initialRoles }) {
    ...
    const { data, setData, put, processing, reset } = useForm({ name: '', permissions: [] });
    ...
    const openEdit = async (role) => {
        ...
        try {
            const res = await fetch(route('settings.roles.index').replace('/settings/roles', '/settings/roles/permissions'));
    ...
    const createRole = () => {
        Inertia.get(route('settings.roles.create'));
    };
        ...
        <SettingsLayout title="Settings">
            ...
            <div className="bg-white p-6 rounded-lg border">
                <div className="flex items-center justify-between mb-4">
                    ...
                    { can('roles.manage') && (
                        <button onClick={createRole} className="px-3 py-2 bg-indigo-600 text-white rounded">Create Role</button>
                    ) }
                </div>
                        ...
                        <tbody>
                            {roles.map(r => (
                                <tr key={r.id} className="border-t">
                                    ...
                                    <td className="p-2">
                                        { can('roles.manage') && (
                                            <button onClick={() => openEdit(r)} className="text-indigo-600 mr-3">Edit</button>
                                        ) }
                                        { can('roles.manage') && (
                                            <button onClick={() => {
                                                if (!confirm('Delete this role?')) return;
                                                Inertia.delete(route('settings.roles.destroy', r.id));
                    ...
                    <div className="bg-white rounded-lg w-full max-w-2xl p-6">
                        ...
                        <form onSubmit={save} className="space-y-4">
                            <div>
                                ...
                                <input value={data.name} onChange={(e) => setData('name', e.target.value)} className="w-full border px-3 py-2 rounded" />
                            </div>
                                        ...
                                        <div key={module} className="p-3 border rounded">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <button type="button" onClick={() => {
                                                    const perms = allPermissions[module].map(p => p.name);
                                                    const hasAll = perms.every(p => (data.permissions || []).includes(p));
                                                    const set = new Set(data.permissions || []);
                                                    if (hasAll) perms.forEach(p => set.delete(p)); else perms.forEach(p => set.add(p));
                                                    setData('permissions', Array.from(set));
